...
app.get('/api/properties', async (req, res) => {
  ...
  try {
    // 1. Fetch Land Registry Data (Optimized Query)
    // We only fetch EXACT matches to avoid timeouts.
    let sparqlQuery = `
    ...
    // Increased timeout to 10 seconds, but simplified logic
    let landRegResponse = await axios.get(landRegUrl, { timeout: 10000 });
    ...
    if (authHeader) {
        try {
            ...
            const epcRes = await axios.get(epcUrl, {
                ...
                timeout: 3000 // Fast timeout for EPC
            });
    ...
    // CRITICAL: Always return an empty array on error, never hang!
    res.json([]); 
  }
});
